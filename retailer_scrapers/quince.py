...
def get_full_resolution_url(url):
    if url.startswith('//'):
        url = 'https:' + url
    return re.sub(r'\?.*$', '', url) + "?w=2000&h=2000&q=90"

def extract_images(page):
    image_urls = set()
    container = page.query_selector('div.sliderImages_container__ZTlvx')
    if container:
        for img in container.query_selector_all('img'):
            for attr in ['src', 'data-src', 'data-zoom', 'data-original']:
                src = img.get_attribute(attr)
                if src and 'images.quince.com' in src:
...
def scrape_product(url):
    try:
        with sync_playwright() as p:
            ...
            title = title_elem.inner_text().strip() if title_elem else "Quince Product"
